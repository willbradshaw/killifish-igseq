#######################################################################
# Compute grouped diversity spectra from pre-computed bootstraps     ##
#######################################################################
...
div <- compute_diversity_grouped(bootstraps, group_within, group_between,
                                 clone_field, min_q, max_q, step_q, ci)
log_done()

# Write output
write_log("Writing diversity spectra to file...")
write_tsv(div$div, outpath_div)
write_tsv(div$test, outpath_test)
log_done()
